...
def venues():
  ...
  group_areas = db.session.query(Venue.city,Venue.state).group_by(Venue.state, Venue.city).all()
  data = []
  for area in group_areas:
    ...
    for venue in venues:
      data[-1]["venues"].append({"id": venue[0], "name": venue[1], "num_upcoming_shows":venue[2]
      })
...
def delete_venue():
  ...
  deleted_item = Venue.query.get(venue_id)
  ...
  try:
    ...
    db.session.rollback()
    flash('please try again. Venue ' + Name + ' could not be deleted.')
...
def delete_artist():
  ...
  deleted_aitem = Artist.query.get(artist_id)
  artistName = deleted_aitem.name

  try:
    db.session.delete(deleted_aitem)
    db.session.commit()
...
def create_show_submission():
  ...
  date_time = request.form['start_time'].split(' ')
  time_list = date_time[0].split('-')
  time_list += date_time[1].split(':') 

  for i in range(len(time_list)):
    time_list[i] = int(time_list[i])

  new_show.start_time = datetime(time_list[0], time_list[1], time_list[2], time_list[3], time_list[4], time_list[5])
  now = datetime.now()
